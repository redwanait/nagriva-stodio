...
const SENDER = "Nagriva <onboarding@resend.dev>";
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const replyTo = preferredContact === "Email" ? email : undefined;
  ...
  try {
    ...
    return res.status(500).json({ error: "Something went wrong while sending your inquiry. Please try again." });
  }
}
...
function clientEmailHtml(input: {
  ...
  const budgetLine = input.budget ? `<div class="row"><span>Budget</span><strong>${escapeHtml(input.budget)}</strong></div>` : "";
  const companyLine = input.company ? `<div class="row"><span>Company / Brand</span><strong>${escapeHtml(input.company)}</strong></div>` : "";
  const phoneLine = input.preferredContact !== "Email" ? `<div class="row"><span>Contact number</span><strong>${escapeHtml(input.phone)}</strong></div>` : "";
  ...
  <body style="margin:0;padding:0;background:#0b0b0b;font-family:Helvetica,Arial,sans-serif;color:#f5f5f5;">
    <div style="max-width:600px;margin:0 auto;padding:40px 24px;background:#111;border:1px solid rgba(245,245,245,.08);border-radius:16px;">
      ...
      <div style="background:#161616;border-radius:12px;padding:20px;margin-bottom:24px;">
        ...
        <div class="row" style="display:flex;justify-content:space-between;padding:6px 0;"><span style="color:#8a8a8a;">What you need</span><strong>${escapeHtml(input.need)}</strong></div>
        ${companyLine}
        ${budgetLine}
        ${phoneLine}
        <div class="row" style="display:flex;justify-content:space-between;padding:6px 0;"><span style="color:#8a8a8a;">Preferred contact</span><strong>${escapeHtml(input.preferredContact)}</strong></div>
      </div>
